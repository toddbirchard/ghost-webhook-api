...
def analytics_week():
    ...
    result = db.insert_dataframe(
        df, table_name="weekly_stats", database_name="analytics", action="replace"
    )
...
def analytics_month():
    ...
    df = results.to_dataframe()
    result = db.insert_dataframe(
        df, table_name="monthly_stats", database_name="analytics", action="replace"
    )
    LOGGER.success(
        f"Successfully inserted {len(result)} rows into monthly_stats table."
    )
    return make_response(result, 200, {"content-type": "application/json"})
